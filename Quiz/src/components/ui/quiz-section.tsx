                                ...
                                <div className="text-center space-y-4">
                                    ...
                                    <div className="flex justify-center space-x-1 mt-6">
                                        {Array.from({ length: 5 }).map((_, i) => (
                                            <div
                                                key={i}
                                                className={`transition-all duration-300 ${i < Math.ceil((finalScore / quizData.length) * 5) ? "animate-pulse" : ""
                                                    }`}
                                                style={{ animationDelay: `${i * 100}ms` }}
                                            >
                                                <Star
                                                    className={`w-10 h-10 ${i < Math.ceil((finalScore / quizData.length) * 5)
                                                            ? "text-yellow-400"
                        ...
                        <div className="grid grid-cols-5 gap-3">
                            {quizData.map((_, index) => (
                                <div key={index} className="text-center">
                                    <div
                                        className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-2 border-2 transition-all duration-300 ${answers[index] === quizData[index].correctAnswer
                        ...
                        <div className="flex justify-between items-center mb-4">
                            {quizData.map((_, index) => (
                                <div key={index} className="flex flex-col items-center space-y-2">
                                    <div
                                        className={`relative transition-all duration-500 ${index <= currentQuestion ? "animate-bounce" : ""
                                            }`}
                                    >
                                        <Gift
                                            className={`w-8 h-8 transition-all duration-300 ${answers[index] !== null
                                                    ? answers[index] === quizData[index].correctAnswer
                                                        ? "text-green-500"
                                                        : "text-red-500"
